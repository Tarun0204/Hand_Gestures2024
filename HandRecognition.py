...
def start():
    ...
    hand_detection = mp_hand_detector.Hands(max_num_hands=1, min_detection_confidence=0.98)
    ...
    while input.isOpened():
        ...
        if results.multi_hand_landmarks:
            ...
            if thumb_index_dist >= 50 and thumb_index_dist <= 150 and index_middle_dist >= 80:  # if the distance between the thumb finger and index finger tips is less than 130 and grater than 50 decreasing the volume
                pyautogui.press('volumedown')
                print(thumb_index_dist)
            elif thumb_index_dist >= 151 and thumb_index_dist <= 300 and index_middle_dist >= 81:  # if the distance between the thumb finger and index finger tips is less than 250 and grater than 130 increasing the volume
